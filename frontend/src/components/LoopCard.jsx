...
if(showComment){
    ...
    return (
        <div className='w-full lg:w-[480px] h-[100vh] flex items-center bg-gray-50 justify-center border-l-2 border-r-2 border-gray-200  relative overflow-hidden'>
...
<div ref={commentRef} className={`absolute z-[200] bottom-0 w-full h-[500px] p-[10px]  rounded-t-4xl bg-white transform transition-transform duration-500 ease-in-out left-0 shadow-2xl shadow-black ${showComment?"translate-y-0":"translate-y-[100%] "}`}>
<h1 className='text-black text-[20px] text-center font-semibold'>Comments</h1>

<div className='w-full h-[350px] overflow-y-auto flex flex-col gap-[20px]'>

    {loop.comments.length==0 && <div className='text-center text-black text-[20px] font-semibold mt-[50px]'>No Comments Yet</div>}

{loop.comments?.map((com,index)=>(
<div className='w-full  flex flex-col gap-[5px] border-b-[1px] border-gray-200 justify-center pb-[10px] mt-[10px]'>
<div className='flex justify-start items-center md:gap-[20px] gap-[10px]'>
          ...
          <div className='w-[150px] font-semibold text-black truncate'>{com.author?.userName}</div>
        </div>
        <div className='text-black pl-[60px]'>{com.message}</div>
</div>
))}
</div>
 <div className='w-full fixed bottom-0 h-[80px] flex items-center justify-between px-[20px] py-[20px] '>
          ...
          <input type="text" className='px-[10px] border-b-2 border-b-gray-200 w-[90%] text-white placeholder:text-black outline-none h-[40px]' placeholder='Write comment...' onChange={(e)=>setMessage(e.target.value)} value={message}/>
...
<div className='flex items-center gap-[5px]'>
        ...
        <FollowButton targetUserId={loop.author?._id} tailwind={"px-[10px] py-[5px] text-white border-2 text-[14px] rounded-full border-white"}/>
         </div>
