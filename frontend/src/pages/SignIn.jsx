...
const handleSignIn=async ()=>{
  ...
  return (
    <div className='w-full h-screen bg-gray-100 flex flex-col justify-center items-center'>
      <div className='w-[90%] lg:max-w-[60%]  h-[600px] bg-white rounded-2xl flex justify-center items-center overflow-hidden border-0 border-[#252e36]'>
...
<button className='w-[70%] px-[20px] py-[10px] bg-gradient-to-b from-blue-500 to-blue-700 transition-transform hover:scale-102 duration-300 text-white font-semibold h-[50px] cursor-pointer shadow rounded-2xl mt-[30px]' onClick={handleSignIn} disabled={loading}>{loading?<ClipLoader size={30} color='white'/>:"Sign In"}</button>
<p className='cursor-pointer text-gray-800' onClick={()=>navigate("/signup")}>Create a new account? <span className='border-b-2 border-b-black pb-[3px] text-black'>Sign Up</span></p>
</div>


{/*Right side Sphere logo */}
...
<p >Make endless serured connections!</p>
</div>
      </div>
    </div>
  )
