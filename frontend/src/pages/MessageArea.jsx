...
return ()=>socket?.off("newMessage")
  ...
  return (
    <div className='w-full h-[100vh] bg-gray-100 relative'>
      ...
      <div className=' w-full flex items-center gap-[15px] px-[20px]  py-[10px] fixed top-0 z-[100] bg-gray-100 '>

<div className=' h-[80px] flex items-center gap-[20px] px-[20px]'>
  <MdOutlineKeyboardBackspace className='text-black cursor-pointer w-[24px]  h-[24px] ' onClick={() => navigate(`/`)} />
   </div>
...
<div className='w-[40px] h-[40px] border-2 border-black rounded-full cursor-pointer overflow-hidden' onClick={()=>navigate(`/profile/${selectedUser.userName}`)}>
      ...
      <div className='text-black text-[18px] font-semibold '>
        <div>{selectedUser.userName}</div>
           <div className='text-[14px] text-gray-500 '>{selectedUser.name}</div>
      </div>
              <p className='text-gray-400 pl-[25%] pt-[50px] hidden md:block '>End to End Encryption 🔐 </p>
      </div>

      
      <div className='w-full h-[80%] pt-[100px] px-[40px] flex flex-col gap-[50px] overflow-auto bg-gray-100 '>
{messages && messages?.map((mess,index)=>
  mess.sender==userData._id?<SenderMessage message={mess}/>:<ReceiverMessage message={mess}/>
)}
      </div>

<div className='w-full h-[80px] fixed bottom-0 flex justify-center items-center bg-gray-100 z-[100]'>
<form className='w-[90%] max-w-[800px] h-[80%] rounded-full border-1 bg-gray-50 flex items-center gap-[10px] px-[20px] relative' onSubmit={handleSendMessage}>
  ...
  <input type="text" placeholder='Message...' className='w-full h-full px-[20px] text-[18px] text-gray-900 outline-0' onChange={(e)=>setInput(e.target.value)} value={input}/>
  <div onClick={()=>imageInput.current.click()}><LuImage className='w-[28px] h-[28px] cursor-pointer text-black'/></div>
